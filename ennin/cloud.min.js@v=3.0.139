...
function Clouder(params) {
    ...
    // init
    // function dbg(reason, i = 0) {
    //     let copy = Object.assign({}, objs[i]);
    //     console.log(reason, copy.id, '.x = ', copy.x);
    //     delete copy.span;
    //     // console.table(copy);
    // }
    ...
    function adjustElems() {
        for (const [i, o] of Object.entries(objs)) {
            let dx = 0, dy = 0, dz = 0;
            for (const [j, oj] of Object.entries(objs)) {
                if (i === j) {
                    continue;
                } // if
                let diffX = o.x - oj.x;
    ...
    function draw() {
        ...
        process(function(o) {
            ...
            if (!filters) {
                ...
                let f = o.span.filters["DXImageTransform.Microsoft.Alpha"];
                if (f) {
                    f.opacity = opa * 100;
                } else {
    ...
    function setClosest(obj) {
        ...
        if (closest != null) {
            // closest.span.style.border = "1px dotted orange";
        } // if
...
class CloudManager {
    ...
    static add(container) {
        ...
        let dataKey = container.getAttribute("data-key");
        let data = CloudManager.data[dataKey];
        let callbackKey = container.getAttribute("data-callback");
        let callback  = CloudManager.callbacks[callbackKey];
        let id = CloudManager.index++;
        CloudManager.clouds[id] = new Clouder({
            container: container,
            tags: data,
            callback: callback,
            // interval: 300,
        ...
        // console.log(CloudManager.clouds[id]);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    window.setTimeout(() => {
        let containers = document.querySelectorAll('div.tag-cloud');
        containers.forEach((container) => {
            CloudManager.add(container);
        });
    }, 1);
});
